  ...
  emitDidChangeTextEvent () {
    ...
    if (this.transactCallDepth === 0) {
      if (this.changesSinceLastDidChangeTextEvent.length > 0) {
        const patch = patchFromChanges(this.changesSinceLastDidChangeTextEvent)
        const compactedChanges = Object.freeze(normalizePatchChanges(patch.getChanges()))
        this.changesSinceLastDidChangeTextEvent.length = 0
        if (compactedChanges.length > 0) {
          this.languageMode.bufferDidChange(compactedChanges)
          for (let displayLayerId in this.displayLayers) {
            this.displayLayers[displayLayerId].bufferDidChange(compactedChanges)
          }
...
class ChangeEvent {
  constructor (buffer, changes) {
    ...
    const start = changes[0].oldRange.start
    const oldEnd = changes[changes.length - 1].oldRange.end
    const newEnd = changes[changes.length - 1].newRange.end
